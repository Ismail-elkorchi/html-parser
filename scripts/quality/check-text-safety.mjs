...
import { nowIso, writeJson } from "../lib/report.mjs";
...
const ROOT_TEXT_FILE_PATTERN =
  /^(?:README\.md|CONTRIBUTING\.md|RELEASING\.md|SECURITY\.md|SUPPORT\.md|LICENSE|package(?:-lock)?\.json|jsr\.json|eslint\.config\.[cm]?js|tsconfig[^/]*\.json|.*\.(?:md|json|ya?ml|toml|mjs|cjs|js|txt)|\.(?:editorconfig|gitignore|gitattributes|npmrc))$/;
...
async function main() {
  ...
  await writeJson("reports/text-safety.json", report);
  ...
  console.error(
    `Text safety check failed: found ${String(violations.length)} violation(s) across ${String(scanTargets.length)} scanned file(s).`
  );
