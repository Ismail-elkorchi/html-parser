...
async function main() {
  ...
  const requirePerformanceQualification = Boolean(
    profilePolicy.requirePerformanceQualification
  );
  const performanceReport = await loadOptionalReport("reports/engine-performance.json");
  const performanceFailures = [];
  if (requirePerformanceQualification) {
    if (!performanceReport) {
      ...
      if (!performanceReport.ok || (performanceReport.failures ?? []).length > 0) {
        performanceFailures.push(...(performanceReport.failures ?? ["Performance report failed"]));
      }
      if ((performanceReport.sampleOrder ?? []).length !== performanceReport.runs) {
        performanceFailures.push("Performance report lacks an exact sample order");
      }
    }
  }
  const performancePass = performanceFailures.length === 0;
  gates.push(makeGate("G-115", "Cross-revision performance", performancePass, {
    required: requirePerformanceQualification,
    report: performanceReport || { missing: true },
  ...
  checkScoreCoherence(
    "performance",
    "requirePerformanceQualification",
    Boolean(profilePolicy.requirePerformanceQualification) && performancePass,
    {
      requirePerformanceQualification,
      performancePass
    }
  ...
  const requireFlagReportMap = {
    ...
    requirePerformanceQualification: ["reports/engine-performance.json"]
  };
